...
![Xoriant / “Decision Trees for Classification: A Machine Learning Algorithm”](dt_example.png)
...
The confusion matrices above show a large decrease in the accuracy, recall, and precision scores for both the training and test sets compared to the metrics from our original model with default parameters; however, we are now no longer overfitting. The accuracy, recall, and precision scores are around 0.3. These trends are also reflected in the new DT. With reducing the depth of the tree, we inevitably reduce the accuracy, recall, and precision scores of the model; however, we also reduce overfitting and increase generalizability. 
...
![TIBCO / “What is a Random Forest?”](rf_example.svg)
...
```{python}
model = RandomForestClassifier(max_depth=4, n_estimators=50, random_state=1234)
model = model.fit(x_train, y_train)
...
Our RF model, given a job type's employment and wage metrics, can classify the associated job type with an accuracy of 0.31. Based on the RF classification results, we can see that our optimized model outperforms the random classifier by about 8 times (accuracy=0.04 vs. accuracy=0.31), which is quite a large improvement from classifying completely randomly. Additionally, our RF classification model also outperforms the results from our decision tree classification results (accuracy=0.31 vs. accuracy=0.28). This result is to be expected,since RFs are composed of multiple decision trees.
